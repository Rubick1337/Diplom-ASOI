...
import { ChallengeImportModal, ChallengeExportModal } from '@/features/ChallengeImportExportModal/ChallengeImportExportModal';
...
function RowMenu({ row, onEdit, onToggle, onDelete, onOpen }: {
    ...
    return (
        <div className="cm-menu" ref={ref}>
            <button ref={btnRef} className="cm-menu__btn" onClick={handleOpen} title="Действия">⋯</button>
            {open && (
                <div className="cm-menu__dropdown" style={{ position: 'fixed', top: pos.top, right: pos.right }}>
                    <button className="cm-menu__item cm-menu__item--open"   onClick={() => act(onOpen)}>↗ Открыть</button>
                    <button className="cm-menu__item cm-menu__item--edit"   onClick={() => act(onEdit)}><img src="/images/edit.png" alt="" />Редактировать</button>
    ...
    return (
        <section className="cm-section">
            ...
            <div className="cm-section__header">
                ...
                <button className="cm-filter-btn cm-filter-btn--ghost" onClick={() => setShowImport(true)}>
                    ↑ Импорт
                </button>
                <button className="cm-filter-btn cm-filter-btn--ghost" onClick={() => setShowExport(true)} disabled={rows.length === 0}>
                    ↓ Экспорт
                </button>
            ...
            {showImport && (
                <ChallengeImportModal
                    ...
                    onImported={() => load(1, search, topicId, limit)}
                />
            )}
