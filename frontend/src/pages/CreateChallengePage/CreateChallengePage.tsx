...
export default function CreateChallengePage() {
    ...
    const handleCreateTopic = async () => {
        ...
        if (!name || topicCreating) return;
        setTopicCreating(true);
        try {
            const created = await ChallengeService.createTopic(name);
    ...
    const handleAiGenerate = async () => {
        ...
        setAiError('');
        try {
            const gen = await ChallengeService.generateChallenge(aiPrompt.trim());
            ...
            setAiError(e?.response?.data?.message || e?.message || 'Ошибка генерации');
        } finally {
            setAiLoading(false);
        }
    };
    ...
    const handlePublish = async () => {
        ...
        if (!verifyResult || !verifyResult.testResults?.every(r => r.status === 'success')) {
            showModal('Проверьте решение', 'Перед публикацией убедитесь, что эталонное решение проходит все тесты');
            return;
        }
        setPublishing(true);
        ...
        try {
            ...
            showModal('Ошибка', 'Не удалось опубликовать задачу.', 'danger');
        } finally {
            setPublishing(false);
        }
    };
    ...
    const someVerified = !!verifyResult && !verifyResult.error;
            ...
            <main className="cc-page">
                ...
                <div className="cc-topbar">
                    <div className="cc-topbar-left">
                        ...
                        <div>
                            <h1 className="cc-title">Создание задачи</h1>
                ...
                {aiOpen && (
                    <div className="cc-ai-panel">
                        <div className="cc-ai-panel-header">
                            <div className="cc-ai-panel-title">
                                <Sparkles size={16} className="cc-ai-icon" />
                                Генерация задачи с ИИ
                            </div>
                            <button className="cc-btn-icon-del" onClick={() => setAiOpen(false)} title="Закрыть">
                                <X size={15} />
                            </button>
                        </div>
                        <p className="cc-ai-hint">
                            Опишите задачу — ИИ заполнит все поля автоматически. Вы сможете отредактировать результат вручную или сгенерировать заново.
                        </p>
                        <textarea
                            className="cc-ai-textarea"
                            rows={3}
                            placeholder="Напр: задача на сортировку массива, сложность средняя; или задача на нахождение наибольшей общей подпоследовательности"
                            value={aiPrompt}
                        ...
                        />
                        {aiError && (
                            <div className="cc-ai-error">
                                <AlertTriangle size={13} /> {aiError}
                            </div>
                        )}
                        <div className="cc-ai-actions">
                            <button
                                className="cc-btn-ai-generate"
                                onClick={handleAiGenerate}
                                disabled={aiLoading}
                            >
                                {aiLoading ? <span className="cc-spinner" /> : <Sparkles size={14} />}
                                {aiLoading ? 'Генерация…' : 'Сгенерировать'}
                            </button>
                            <span className="cc-ai-tip">Ctrl+Enter</span>
                        </div>
                    </div>
                )}
                                        ...
                                        {topicOpen && (
                                            <div className="select-options">
                                                <div className="cc-topic-search" onClick={e => e.stopPropagation()}>
                                                    <input
                                                        autoFocus
                                                        placeholder="Поиск или создать тему…"
                                                        value={topicInput}
                                                ...
                                                {(() => {
                                                    ...
                                                    const exactMatch = topics.some(t => t.name.toLowerCase() === topicInput.toLowerCase().trim());
                                                    return (
                                                        <>
                                                            {filtered.map(t => (
                                                            ...
                                                            {topicInput.trim() && !exactMatch && (
                                                                <div
                                                                    className="option sort-option cc-topic-create"
                                                                    onClick={e => { e.stopPropagation(); handleCreateTopic(); }}
                                                                >
                                                                    {topicCreating
                                                                        ? <span className="cc-spinner cc-spinner-xs" />
                                                                        : <Plus size={12} />}
                                                                    Создать «{topicInput.trim()}»
                                                                </div>
                                                            )}
                                                        </>
                                                    );
                        ...
                        <section className="cc-card cc-verify-card">
                            ...
                            {verifyResult && !verifyResult.error && (
                                <div className="cc-verify-summary">
                                    ...
                                    <div className={`cc-verify-verdict ${allVerified ? 'ok' : 'fail'}`}>
                                        {allVerified
                                            ? <><CheckCircle2 size={15} /> Все тесты пройдены — задача готова к публикации</>
                            ...
                            {!verifyResult && !verifyError && (
                                <div className="cc-verify-placeholder">
                                    Напишите решение, которое проходит все тесты, и нажмите «Запустить» перед публикацией
                                </div>
                            )}
                        </section>
                    </div>
